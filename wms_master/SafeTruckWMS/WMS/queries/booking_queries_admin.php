  ...
  function ViewAllPendingBookings($workshop_lng, $workshop_ltd)
  {
    ...
    ( 6371 * acos( cos( radians(:workshop_ltd) ) * cos( radians( customer_ltd ) ) * cos( radians( customer_lng ) - radians(:workshop_lng) ) + sin( radians(:workshop_ltd2) ) * sin(radians(customer_ltd)) ) ) AS distance
    FROM `bookings` WHERE accepted_status = 'pending' AND workshop_id = 1 HAVING distance < 50 ORDER BY time_created DESC");
    $stmt->bindParam(":workshop_lng", $workshop_lng);
    $stmt->bindParam(":workshop_ltd", $workshop_ltd);
    $stmt->bindParam(":workshop_ltd2", $workshop_ltd);
 ...
 try{
    ...
    $stmt = $conn->prepare("SELECT * FROM Bookings WHERE accepted_status = 'pending' AND workshop_id = :workshop_id");
  ...
  function ViewAllAcceptedBookings($workshop_id)
  {
    ...
    $stmt = $conn->prepare("SELECT * FROM Bookings WHERE accepted_status = 'accepted' AND workshop_id = :workshop_id");
    $stmt->bindParam(":workshop_id", $workshop_id, PDO::PARAM_INT);
    $stmt->execute() or die($conn->error);
    $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $conn = null;
  ...
  function ToggleAcceptBooking($booking_id, $workshop_id)
  {
    ...
    try{
      if (gettype($result) == 'boolean'){
        ...
        (workshop_id , customer_id, vehicle_plate, vehicle_make, description)
        VALUES (:workshop_id, :customer_id, :vehicle_plate, :vehicle_make, :description)");
  ...
  function ToggleDeleteBooking($booking_id)
  {
    ...
    $stmt = $conn->prepare("DELETE FROM Bookings WHERE booking_id = :booking_id AND accepted_status = 'accepted'");
    $stmt->bindParam(":booking_id", $booking_id, PDO::PARAM_INT);
    $stmt->execute() or die($conn->error);
  }
